...
##чтобы была минимальной разность между количествами точек, лежащих внутри и
##вне треугольника с вершинами в выбранных точках.
##                                                Белоусова Ю. ИУ7-11Б
...
##def pointinside(Ax,Ay,Bx,By,Cx,Cy,Mx,My):
##    a=(Ax-Mx)*(By-Ay)-(Bx-Ax)*(Ay-My)
##    b=(Bx-Mx)*(Cy-By)-(Cx-Bx)*(By-My)
##    c=(Cx-Mx)*(Ay-Cy)-(Ax-Cx)*(Cy-My)
##    if (a>0 and b>0 and c>0) or (a<0 and b<0 and c<0) or a==0 or b==0 or c==0:
##        return 1
##    else:
##        return 0
...
##                                    if pointinside(xarr[i],yarr[i],xarr[j],yarr[j],xarr[k],yarr[k],xarr[r],yarr[r])==1:
##                                        inside+=1
##                                    else:
##                                        outside+=1
...
def draw():
    ...
    if dx>dy:
        ...
        delta=(n1)/dy
    for i in range(l):
        x=(xarr[i]-xmin)*delta+10
        y=n1-(yarr[i]-ymin)*delta+10
        canv.create_oval(x,y,x,y,width=5)
        string = '(' + str(xarr[i]) + ';' + str(yarr[i]) + ')'
        canv.create_text(x, y, fill = 'black', text = string)
    
    x0=(-xmin)*delta+10
    y0=n1-(-ymin)*delta+10
    x1=(10000-xmin)*delta+10
    y1=n1-(10000-ymin)*delta+10
    x2=(-10000-xmin)*delta+10
    y2=n1-(-10000-ymin)*delta+10
    canv.create_line(x2,y0,x1,y0,fill='yellow')#оси
    canv.create_line(x0,y2,x0,y1,fill='yellow')
    x0=(xarr[idiff]-xmin)*delta+10
    y0=n1-(yarr[idiff]-ymin)*delta+10
    x1=(xarr[jdiff]-xmin)*delta+10
    y1=n1-(yarr[jdiff]-ymin)*delta+10
    x2=(xarr[kdiff]-xmin)*delta+10
    y2=n1-(yarr[kdiff]-ymin)*delta+10
...
btn3.grid(row=27,column=0)
#Область вывода
lst=Listbox(root,height=20,width=25)
scrollbar=lst.yview
